...
const { spinner, access, copyFile, generateError, config } = require(`../utils`)

const generateDatabase = async () => {
	return `${config.projectType === `ts` ? `export default` : `module.exports = `} ({ env }) => ({
	connection: {
...
const checkAndBackupDB = async () => {
	const databasePath = path.join(process.cwd(), `config`, `database.${config.projectType}`)
