...
const { spinner, replace, chalk, generateError, config } = require(`../../utils`)
...
const appendEnv = async () => {
	...
	await readFile(`.env`, `utf8`, async (err, data) => {
		...
		writeLine(
			`NODE_ENV=${
				config.env.toLowerCase() === `both` || config.env.toLowerCase() === `development`
					? `development`
					: `production`
			}`,
		)
		writeLine(`DATABASE_CLIENT=${config.dbtype === `postgresql` ? `postgres` : `mysql2`}`)
		writeLine(`# @strapi-community/dockerize end variables \n`)
		spinner.stopAndPersist({
			symbol: `🕵️`,
			text: `  Added ${chalk.bold.blue(`Dockerize`)} variables in ${chalk.yellow(`.env`)} \n`,
		})
	})
}
...
const envUpdate = async (env) => {
	const options = {
		...
		to: [
			`${env.toLowerCase()}`,
			`development`,
			`DATABASE_CLIENT=${config.dbtype.toLowerCase() === `postgresql` ? `postgres` : `mysql2`}`,
	...
	try {
		...
		spinner.stopAndPersist({
			symbol: `🕵️`,
			text: `  Updated ${chalk.bold.blue(`Dockerize`)} variables in ${chalk.yellow(`.env`)} \n`,
		})
	} catch (error) {
