...
const createDockerFiles = async () => {
	...
	try {
		...
		if (config.env === `production` || config.env === `both`) {
			...
			await outputFile(path.join(config.outDir, `Dockerfile.prod`), prodTemplate)
		}

		spinner.stopAndPersist({
			symbol: `🐳`,
			text: ` ${chalk.bold.blue(`Dockerfile`)} for ${chalk.yellow(
				config.env === `both` ? `development and production` : config.env.toUpperCase(),
			)} added \n`,
		})
	} catch (error) {
		generateError(error)
	...
	await copyFile(`${config.templateDir}/.dockerignore`, `${config.outDir}/.dockerignore`)
