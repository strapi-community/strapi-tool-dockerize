...
const detectPackageManager = async () => {
	...
	if (!config.quickStart) {
		...
		if (installedPackages.length === 2) {
			...
			spinner.stopAndPersist({
				symbol: `\n📦`,
				text: `Using ${chalk.bold.blueBright(config.packageManager.toUpperCase())} \n`,
			})
			return
		} else {
			try {
			...
			spinner.stopAndPersist({
				symbol: `📦`,
				text: ` ${chalk.bold.yellow(config.packageManager.toUpperCase())} detected \n`,
			})
			return
		}
	}
...
const detectStrapiProject = async () => {
	...
	try {
		...
		if (Object.prototype.hasOwnProperty.call(packageObj, `dependencies`)) {
			if (Object.prototype.hasOwnProperty.call(packageObj.dependencies, `@strapi/strapi`)) {
				const strapiVersion = packageObj.dependencies[`@strapi/strapi`]
				spinner.stopAndPersist({
					symbol: `🚀`,
					text: ` ${chalk.bold.yellowBright(`Strapi ${strapiVersion}`)} detected \n`,
				})
				return true
			} else {
				spinner.stopAndPersist({
