...
describe("docker-compose template", () => {
	...
	describe("environment-specific compose files", () => {
		it("generates docker-compose.yml for development environment", async () => {
			const output = await renderTemplate("docker-compose", sqliteContext({ environment: "development" }))
		...
		it("generates docker-compose.prod.yml for production environment", async () => {
			const output = await renderTemplate("docker-compose", sqliteContext({ environment: "production" }))
		...
		it("generates both docker-compose.yml and docker-compose.prod.yml for both environment", async () => {
			const devOutput = await renderTemplate("docker-compose", sqliteContext({ environment: "development" }))
			const prodOutput = await renderTemplate("docker-compose", sqliteContext({ environment: "production" }))
		...
		it("docker-compose.yml references Dockerfile for development", async () => {
			const output = await renderTemplate("docker-compose", postgresContext({ environment: "development" }))
			expect(output).toContain("dockerfile: Dockerfile")
			expect(output).not.toContain("Dockerfile.prod")
		})

		it("docker-compose.prod.yml references Dockerfile.prod for production", async () => {
			const output = await renderTemplate("docker-compose", postgresContext({ environment: "production" }))
			expect(output).toContain("dockerfile: Dockerfile.prod")
		})

		it("both compose files have correct volume mounts", async () => {
			const devOutput = await renderTemplate("docker-compose", postgresContext({ environment: "development" }))
			...
			const prodOutput = await renderTemplate("docker-compose", postgresContext({ environment: "production" }))
